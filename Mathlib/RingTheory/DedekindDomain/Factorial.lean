...
def fixedDivisor [Semiring R] (𝒻 : R[X]) : Ideal R := Ideal.span <| 𝒻.eval '' S
...
example : f.fixedDivisor univ = Ideal.span {2} := by
  ...
  have principal : f.fixedDivisor univ |>.IsPrincipal := ⟨sorry⟩
  have : 2 ∈ f.fixedDivisor univ := sorry
  sorry

section UniqueFactorizationMonoid

variable [CommRing R] [IsDomain R] [UniqueFactorizationMonoid R]

theorem fixedDivisor_eq_gcd (𝒻 : R[X]) : 𝒻.fixedDivisor = (𝒻.eval '' S).gcd := sorry
